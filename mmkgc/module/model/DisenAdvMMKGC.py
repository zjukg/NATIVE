...
class DisenAdvMMKGC(Model):
    ...
    def __init__(
        ...
        super(DisenAdvMMKGC, self).__init__(ent_tot, rel_tot)
    ...
    def forward(self, data):
        ...
        score_joint = self.margin - self._calc(h_joint, t_joint, r, mode)
        
        disen_loss = self.get_disentangle_loss(h, h_img_emb, h_text_emb)
        return score_joint, disen_loss
    ...
    def get_batch_ent_multimodal_embs(self, data, rel):
        s = self.ent_embeddings(data)
        v = self.img_proj(self.img_embeddings(data))
        t = self.text_proj(self.text_embeddings(data))
        # rg = self.rel_gate(rel)
        return s, v, t
