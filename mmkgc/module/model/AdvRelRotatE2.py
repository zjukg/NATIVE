...
class AdvRelRotatE2(Model):
    ...
    def __init__(
        ...
        super(AdvRelRotatE2, self).__init__(ent_tot, rel_tot)
        ...
        self.audio_proj = nn.Linear(128, self.dim_e)
        self.audio_embeddings = nn.Embedding.from_pretrained(audio_emb).requires_grad_(True)
        self.video_proj = nn.Linear(2048, self.dim_e)
        self.video_embeddings = nn.Embedding.from_pretrained(video_emb).requires_grad_(True)
    ...
    def get_joint_embeddings(self, es, ei, et, ea, ev, rg):
        ...
        i = self.gated_fusion(ei, rg)
        t = self.gated_fusion(et, rg)
        a = self.gated_fusion(ea, rg)
        v = self.gated_fusion(ev, rg)
        e = torch.stack((s, i, t, a, v), dim=1)
        u = torch.tanh(e)
