...
class AdvRelJointRotatE(Model):
    ...
    def __init__(
        ...
        super(AdvRelJointRotatE, self).__init__(ent_tot, rel_tot)
    ...
    def get_joint_embeddings(self, es, ev, et, rg):
        # s = self.gated_fusion(es, rg)
        # v = self.gated_fusion(ev, rg)
        # t = self.gated_fusion(et, rg)
    ...
    def forward(self, data):
        ...
        h_joint, _ = self.get_joint_embeddings(h, h_img_emb, h_text_emb, rg)
        t_joint, _ = self.get_joint_embeddings(t, t_img_emb, t_text_emb, rg)
    ...
    def get_fake_score(
        ...
        h_joint, _ = self.get_joint_embeddings(h, h_img_emb, h_text_emb, rg)
        t_joint, _ = self.get_joint_embeddings(t, t_img_emb, t_text_emb, rg)
        h_fake, _ = self.get_joint_embeddings(h, fake_hv, fake_ht, rg)
        t_fake, _ = self.get_joint_embeddings(t, fake_tv, fake_tt, rg)
    ...
    def predict(self, data):
        score, scores = self.forward(data)
        score += scores[0]
        return -score.cpu().data.numpy()
    
    def predict_single_modal(self, data):
        score, _, _ = self.forward(data)
        return -score.cpu().data.numpy()
