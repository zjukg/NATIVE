...
class AdvRelRotatE3(Model):
    ...
    def __init__(
        ...
        super(AdvRelRotatE3, self).__init__(ent_tot, rel_tot)
        ...
        self.rel_gate_s = nn.Embedding(self.rel_tot, self.dim_e)
        nn.init.uniform_(
            tensor=self.rel_gate_s.weight.data,
            a=-self.ent_embedding_range.item(),
            b=self.ent_embedding_range.item()
        )
        self.rel_gate_v = nn.Embedding(self.rel_tot, self.dim_e)
        nn.init.uniform_(
            tensor=self.rel_gate_v.weight.data,
            a=-self.ent_embedding_range.item(),
            b=self.ent_embedding_range.item()
        )
        self.rel_gate_t = nn.Embedding(self.rel_tot, self.dim_e)
        nn.init.uniform_(
            tensor=self.rel_gate_t.weight.data,
            a=-self.ent_embedding_range.item(),
            b=self.ent_embedding_range.item()
        )
    ...
    def get_joint_embeddings(self, es, ev, et, batch_r):
        rs = self.rel_gate_s(batch_r)
        rv = self.rel_gate_v(batch_r)
        rt = self.rel_gate_t(batch_r)
        s = self.gated_fusion(es, rs)
        v = self.gated_fusion(ev, rv)
        t = self.gated_fusion(et, rt)
        e = torch.stack((s, v, t), dim=1)
        u = torch.tanh(e)
    ...
    def forward(self, data):
        ...
        h_joint = self.get_joint_embeddings(h, h_img_emb, h_text_emb, batch_r)
        t_joint = self.get_joint_embeddings(t, t_img_emb, t_text_emb, batch_r)
    ...
    def get_fake_score(
        ...
        h_joint = self.get_joint_embeddings(h, h_img_emb, h_text_emb, batch_r)
        t_joint = self.get_joint_embeddings(t, t_img_emb, t_text_emb, batch_r)
        h_fake = self.get_joint_embeddings(h, fake_hv, fake_ht, batch_r)
        t_fake = self.get_joint_embeddings(t, fake_tv, fake_tt, batch_r)
